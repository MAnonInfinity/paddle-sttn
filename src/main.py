...
class SubtitleRemover:
    ...
    def color_correct(out, mask_full, sy0, sy1, sigma=None):
        ...
        if not sm.any():
            return
        # Shift only the LOW-frequency colour of the filled pixels toward the
        # local background colour (estimated by blurring the non-masked
        # neighbourhood), preserving the fill's detail. (BGR low-freq match — a LAB
        # chroma-replace variant looked worse on textured backgrounds.)
