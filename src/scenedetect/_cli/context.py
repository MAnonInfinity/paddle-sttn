...
class CliContext:
    ...
    def handle_options(
        ...
        self._open_video_stream(
            ...
            framerate=framerate,
            src=self.config.get_value("global", "src", src, ignore_default=True))
    ...
    def _open_video_stream(self, input_path: AnyStr, framerate: Optional[float],
                           src: Optional[str]):
        if '%' in input_path and src != 'opencv':
            raise click.BadParameter(
                'The OpenCV src (`--src opencv`) must be used to process image sequences.',
                param_hint='-i/--input')
        ...
        try:
            if src is None:
                src = self.config.get_value('global', 'src')
            else:
                if not src in AVAILABLE_BACKENDS:
                    raise click.BadParameter(
                        'Specified src %s is not available on this system!' % src,
                        param_hint='-b/--src')
            # Open the video with the specified src, loading any required config settings.
            if src == 'pyav':
                self.video_stream = open_video(
                    path=input_path,
                    framerate=framerate,
                    src=src,
                    threading_mode=self.config.get_value('src-pyav', 'threading-mode'),
                    suppress_output=self.config.get_value('src-pyav', 'suppress-output'),
                )
            elif src == 'opencv':
                self.video_stream = open_video(
                    path=input_path,
                    framerate=framerate,
                    src=src,
                    max_decode_attempts=self.config.get_value('src-opencv',
                                                              'max-decode-attempts'),
                )
            # Handle srcs without any config options.
            else:
                self.video_stream = open_video(
                    path=input_path,
                    framerate=framerate,
            ...
            logger.debug('Video opened using src %s', type(self.video_stream).__name__)
            ...
            raise click.BadParameter(
                ...
                (' using %s src' % src if src else '', str(ex)),
                param_hint='-i/--input') from ex
