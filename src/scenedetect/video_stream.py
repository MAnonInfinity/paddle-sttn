...
You can also optionally specify a framerate and a specific src library to use. Unless specified,
OpenCV will be used as the video src. See :mod:`scenedetect.srcs` for a detailed example.
...
class VideoOpenFailure(Exception):
    """Raised by a src if opening a video fails."""

    # pylint: disable=useless-super-delegation
    def __init__(self, message: str = "Unknown src error."):
        """
        Arguments:
            message: Additional context the src can provide for the open failure.
        """
...
class FrameRateUnavailable(VideoOpenFailure):
    """Exception instance to provide consistent error messaging across srcs when the video frame
...
class VideoStream(ABC):
    """ Interface which all video srcs must implement. """
    ...
    def BACKEND_NAME() -> str:
        """Unique name used to identify this src. Should be a static property in derived
        classes (`BACKEND_NAME = 'src_identifier'`)."""
    ...
    def seek(self, target: Union[FrameTimecode, float, int]) -> None:
        ...
        May not be supported on all src types or inputs (e.g. cameras).
...
# TODO(0.6.3): Add a StreamJoiner class to concatenate multiple videos using a specified src.
