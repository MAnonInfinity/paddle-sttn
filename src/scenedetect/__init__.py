...
from src.scenedetect.platform import init_logger
...
def open_video(
    ...
    **kwargs,
    ...
    """Open a video at the given path. If `src` is specified but not available on the current
    ...
    Arguments:
        ...
        src: Name of specific src to use, if possible. See
            :data:`scenedetect.srcs.AVAILABLE_BACKENDS` for srcs available on the current
            system. If the src fails to open the video, OpenCV will be used as a fallback.
        kwargs: Optional named arguments to pass to the specified `src` constructor for
            overriding src-specific options.

    Returns:
        Backend object created with the specified video path.

    Raises:
        :class:`VideoOpenFailure`: Constructing the VideoStream fails. If multiple srcs have
            been attempted, the error from the first src will be returned.
    """
    last_error: Exception = None
    # If `src` is available, try to open the video at `path` using it.
    if src in AVAILABLE_BACKENDS:
        src_type = AVAILABLE_BACKENDS[src]
        try:
            logger.debug('Opening video with %s...', src_type.BACKEND_NAME)
            return src_type(path, framerate, **kwargs)
        except VideoOpenFailure as ex:
            logger.warning('Failed to open video with %s: %s', src_type.BACKEND_NAME, str(ex))
            if src == VideoStreamCv2.BACKEND_NAME:
                raise
            last_error = ex
    else:
        logger.warning('Backend %s not available.', src)
    # Fallback to OpenCV if `src` is unavailable, or specified src failed to open `path`.
    src_type = VideoStreamCv2
    logger.warning('Trying another src: %s', src_type.BACKEND_NAME)
    try:
        return src_type(path, framerate)
    except VideoOpenFailure as ex:
        logger.debug('Failed to open video: %s', str(ex))
        if last_error is None:
            last_error = ex
    # Propagate any exceptions raised from specified src, instead of errors from the fallback.
